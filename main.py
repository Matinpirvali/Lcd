...
def display_image(file_path):
    ...
    send_data([0x00, 0x00, 0x00, 0xEF])  # محدوده X (0 تا 239)
    send_command(ILI9341_PASET)
    send_data([0x00, 0x00, 0x01, 0x3F])  # محدوده Y (0 تا 319)
    send_command(ILI9341_RAMWR)

    with open(file_path, "rb") as f:
        while chunk := f.read(4096):  # خواندن و ارسال در بسته‌های 4096 بایتی
            spi.writebytes(list(chunk))

# اجرای برنامه
init_display()
display_image("sample_image.rgb")  # نمایش تصویر
print("تصویر روی LCD نمایش داده شد!")
